...
?>
  ...
  <br><br><br><br><br>
      ...
      include "dbLogin.php";
      $db = DBLogin();
      $string = $_POST["selectComplex"];
      $array = explode("|" , $string);

      $addMovie = $_POST["selectMovie"];
      $addTheatre = (int)$array[0];
      $addComp = $array[1];
      $addDay = $_POST["addDay"];
      ...
      $q = "SELECT MaxSeats
              FROM theatre
              WHERE Complex = '$addComp' 
              AND TheatreNum = '$addTheatre'";
      $resultq = $db->query($q);
      while ($row = $resultq->fetch_assoc()) {
        $NumSeats = $row["MaxSeats"];
        // Add Showing
        $sql1 = "INSERT INTO showing (Complex, Theatre, StartTime, Day, NumSeats, Movie)
               VALUES ('$addComp', $addTheatre, '$addTime', '$addDay', $NumSeats, '$addMovie')";
        if ($db->query($sql1) == TRUE) {
          echo "<p>Showing successfully added!</p>";
        }
        else 
        {
